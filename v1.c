...
int main() {
    ...
    while (tecla != 10) // Enter
    {
        ...
        if (timerTimeOver() == 1) {
            ...
            // Verificar a pontuação para atualizar o objeto
            if (pontos >= 7 && strcmp(ayla.nome, "Ayla") == 0) {
                strcpy(ayla.nome, "a y l a");
            }

            // Impressões
            imprimirPontuacao(pontos);

            // Atualizar objeto com base na pontuação
            if (pontos >= 7) {
                imprimirObjeto(ayla.x, ayla.y, ayla.nome, 6);
            } else {
